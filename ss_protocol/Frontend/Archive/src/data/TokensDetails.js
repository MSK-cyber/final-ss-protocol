...
export const TokensDetails = () => {
  ...
  const combined = [...staticTokens, ...dynamicTokens, ...deployedTokens];
  const seen = new Set();
  ...
  let tokens = data.map((token) => {
    ...
    if (token.isDeployed || isFromDeployedList) {
      ...
      return {
        ...
        Cycle: swap.CurrentCycleCount?.[token.name]
          ? (swap.CurrentCycleCount[token.name] === 'not started' ? 'Not Started' : swap.CurrentCycleCount[token.name] + 1)
          : '0/20',
      };
    }
